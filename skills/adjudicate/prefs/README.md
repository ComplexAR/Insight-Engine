# Adjudication preferences — code-backed switchable settings (v2)
...
v2 replaces the single standing opt-out with a **13-key switchable settings block** — every
setting is operator-changeable, standing and per-run; nothing is centrally locked (personal-use
philosophy). The old opt-out survives as two of the keys plus a derived mirror.
...
re-opens a governance block, or raises cross-lab breadth** requires a deliberate `--confirm`
...
| `crosslab_model` | string | gpt-5.6-sol | which cross-lab model rung A targets (exported as `CROSSLAB_MODEL`); verified on next preflight |
