...
- **An absence claim without a method line is a lead, not a finding.** Any adjudicator claim that something is *not* present - in the packet, the record, or the retrievable evidence - carries one re-runnable line saying where it looked and how: *"Checked: `grep -rn "<x>" <path>` -> no matches."* or *"Checked: fetched `<url>` on `<date>`; page states currency "<verbatim stamp>"; `<thing>` not present."* A positive finding quotes its own proof; an absence has nothing to quote, which is why the burden falls here and on nothing else - requiring it of positive findings would be pure overhead. **The method must cover the full scope the claim denies:** one file does not establish "nowhere in the corpus", and a working tree establishes nothing about a built package. Where method and claim disagree in scope, read the claim down to the method. Where currency is in play, the line carries the source's own currency statement verbatim, never the retrieval date. **Put this rule in the packet preamble and the Verdict Card, not only in your own prompt** - a rung-A adjudicator reads only the packet, so a rule kept here is inert for exactly the runs that most need it. It makes a negative finding cheap to check; it does not make it right, and no error rate is scored.
- **Every dispatch prompt states the host-environment constraints the reviewer works under**, with the reason recorded. A dispatched reviewer inherits none of the operator's standing rules and cannot infer them.
