...
**Corpus version described:** compiled against insight-engine **v1.0.0** and **not re-reviewed since**; the plugin is at **1.3.1** as at 2026-09-01. The identifier scheme is unchanged since v0.1.22. Entries carrying a version claim should be read with that gap in mind; the Archetype entry was corrected on 2026-09-01 and is current. The identifier scheme in Section 4 is the shipped scheme, fixed by the specification in `skills/analyse/SKILL.md`.
...
**Gate Ledger** — The record of the forced judgement gate, reproduced verbatim in the deliverable. Added to this edition 2026-08-30. One line per question, using the `GateQ` series (Section 4): each carries `POSITION` with the operator's own line quoted, `DEFERRED` with the date logged and the default assumption named, or `OPEN`. A `STATUS:` line closes it. **The ledger is what the compliance receipt's gate counts are checked against**, and neither is worth much alone: the receipt without a ledger is an unaudited assertion, and the ledger without a receipt leaves a reader to do the arithmetic. `n` counts every line no longer `OPEN`, whether resolved by a position or a logged deferral, and `p + d = n`. **A deferral counts as resolved** — deferring is a legitimate answer the protocol grants — which is why the receipt reports the split rather than one number meaning two things. **While any line reads `OPEN`, the brief is withheld.** A gate the operator refused outright reads `STATUS: refused` and never `resolved`, because a refused gate and a resolved gate are different states and the record exists to tell them apart. **A third state, standing deferral**, covers a blanket instruction rather than an answer question by question — "defer everything", or an instruction given mid-gate to produce the brief and take the rest later. Its receipt carries `— standing` after the deferred count, and **`p` may be greater than zero**, because the instruction can arrive after some questions have been answered. Within the ledger, a question the operator saw and deferred reads `DEFERRED — … — standing deferral`; one they **never saw** reads `DEFERRED — … — standing deferral, question not put`. Both count toward `d`, and the distinction is recorded because a choice never offered is not a choice declined. *Widened 2026-08-31: the rule previously fixed `p` at zero "by construction", which a conformance run the same day showed to be too narrow.* Authority: `analyse` Step 8; Portable Edition `templates/gate-ledger.md` and P1, which are canonical for the format.
...
**Resolve (of a citation)** — A citation *resolves* when following it arrives at the thing it names. Added 2026-08-30 with v1.0.1. Before any `[V1]`, the retrieved source's own identifiers are checked against the citation about to be given: for a judgment the court and neutral citation number, for legislation the Act, year and section, for a study the journal, year and DOI or title, for an official statistic the publishing body and release. Three failures are distinguished, and they grade differently. **(a) The citation resolves to a different real source** — the mismatch is itself a finding; the claim *as cited* caps at `[U]`, situation 1, and the weakness is recorded as *citation does not resolve to the source named*, quoting both the citation and what the link returns. The found source is never silently substituted for the cited one; if it supports the claim on its own terms it is cited under its own identifiers and graded on its merits, and the mismatch record stays in the deliverable either way. The cap is not tier-scoped: a mismatch found on a `[V2]` or `[V3]` citation caps that citation's support the same way. **(b) The citation does not resolve at all** — a dead link or error page allows no identity check, so `[V1]` is unavailable; grade on what was actually read, `[V3]` at best, weakness *source named but not locatable*, dead URL recorded. **(c) The citation resolves to the right source whose content cannot be read** — a paywall or login; identity may be confirmed from what the page shows, but the grade rests on what was read: supporting passage retrieved, grade normally and record the barrier; abstract, headnote or landing page only, `[V2]` at best under the précis rule; nothing bearing on the claim read, `[U]`. Authority: `verify`; `analyse` Step 4; Architecture v1.0.1. Origin: a shipped defect — a finding graded `[V1]` as "the judgment itself" cited a Court of Appeal neutral citation against an Administrative Court URL with a different case number, and passed all twenty-three conformance probes then in the battery.
...
**Compliance receipt** — The single line that opens every full-pass deliverable, before anything else. Added to this edition 2026-08-30. It names, in fixed order: the edition and version, the host, **the model that graded, as it identifies itself**, the four host capabilities, the declared date, which mechanism held the gate (`enforce:`), the gate counts, whether verification ran, and whether the legend is attached. **The prefix `IE` names this plugin edition; `IE-P` is the Portable Edition.** The two number themselves independently, so the prefix is what makes a version comparison meaningful. The receipt exists so a reader can check the run rather than trust it: **a receipt reading `gate: 3/5 resolved` above a finished brief says the engine broke its own rule**, and the document is invalid. The model is named because grading is model judgement — the specification fixes what the tiers mean, not how reliably a given model applies them — and it is recorded as a claim rather than a fact, because a provider may route a query to a model other than the one selected. Field variants exist for a standalone verification (`gate: n/a`), a refused gate, a standing deferral, partial verification, a render, and an unverified run; a triage-down-scoped short answer carries no receipt at all. Authority: `analyse` Step 9 item 0; `verify`; `render`; Portable Edition Core §10, which is canonical for the format.
...
**Archetype** — A named recurring feedback structure from the systems literature (Shifting the Burden, Limits to Growth, Escalation, Fixes that Fail, and the rest of the standard catalogue). **The shipped engine deliberately carries no archetype vocabulary and names none.** Method: `grep -ci archetype skills/*/SKILL.md` returns 0 for all five skills; the word appears in this repository only in this document and in `Foundations.md`. Across seven runs on two cases the engine named no archetype. **The archetype-slot proposal was tested and refused.** `T-ARCH-001` showed that a two-sentence slot moved naming from 0 of 5 to 5 of 5 on a case the catalogue fits; `T-ARCH-002` then put the same slot to a case it does not fit, on 25 August 2026, and **0 of 5 runs declined** although declining was offered as a first-class answer. Ten blind assessments of the resulting mappings returned **five strained, five absent, none genuine**, and the registered refusal condition fired 5 of 5 against a threshold of 2. The grade ceiling is not what failed — dominance, trajectory and remedy stayed at `[U]` throughout. What failed is the premise of naming only when matched: **it requires the engine to determine reliably that nothing matches, and that is the thing the test refused.** **Operator-carried, run-local use remains permitted** — the guarded clause may be placed in a dispatch prompt for a single run, where it is visible in the deliverable and reversible. The reserved form `Archetype: <name>`, the requirement that it be anchored element by element in the run's own drawn links, and the ceiling sentence (catalogue dynamics and remedies lift no grade and set no ranking) are retained as **the record of what such an operator-carried clause must contain — not as a pending build.** Reopening has one defined entry: a wording that produces a decline on a non-matching case, with `T-ARCH-002` as its control. Authority: T-ARCH-001 RESULT §7; T-ARCH-002 RESULT §10; ADJ-007B; ADJ-007 Reconciliation; PR-HZN-001 RESULT §3.3. *(This entry read "a designed rule, not shipped in any release to date" until 2026-09-01, and described the engine as at v0.1.21. Both were stale: the proposal had been refused on 25 August and the plugin was at 1.3.1. A reader was told a question was open that had been closed.)*
