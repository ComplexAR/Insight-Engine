...
- **The disconfirmation pass.** For every claim the conclusion materially rests on, a *second* search is run at the claim's negation ("X is false / disputed / overstated"). Credible counter-evidence downgrades the claim to `[V3]` or `[N]` and records the dispute; a genuine negation search that finds nothing marks the claim *survived disconfirmation* — but only where the corroboration spans at least two independent roots; a single-root field is marked *unopposed — single-root record*, not survived. Survival across independent roots is itself a real signal. This is the engine's answer to the field's most common failure mode, which is the opposite: confirmation bias.
